...
`@ffmpeg-installer/ffmpeg`) and re-serializes, so WPPConnect Server's own
`version` field keeps reflecting the tag actually cloned. Both `setup_api.py`
and `api_setup.py` carry that list and a test holds them equal.

**Never pin `@wppconnect-team/wppconnect`.** It was once forced to an exact
version that went stale within days — that package releases several times a
week, and wppconnect-server's own `package.json` had already moved to a newer
one, so WinZapp silently ran an incompatible pairing implementation. A test
exists solely to assert the pin stays gone.
...
wppconnect_welcome_layer_patch.py   welcome.layer.js
```

(Note: CLAUDE.md still describes only the first and third — it predates the
other two.)
