...
```
Tests cover `client/core/database.py` (async SQLite layer) and small islands of pure logic pulled out of `main.py`/`core/notification_manager.py` (e.g. `tests/test_sender_names.py`, `tests/test_notifications.py`) by binding their unbound methods onto a plain stub object. There are no tests for the wx UI in bulk. Async tests use `pytest-asyncio` in `auto` mode — no `@pytest.mark.asyncio` decorator needed, just declare test functions `async def`.
