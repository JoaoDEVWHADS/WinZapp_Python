...
The WPPConnect session token is DPAPI-protected (`win32crypt.CryptProtectData`/`CryptUnprotectData`, tied to the current Windows user account), not stored as plain JSON. `MainWindow._get_wa_token()`/`_set_wa_token()` are the *only* sanctioned access points — every read/write of `settings["privateinfo"]["WA_token"]`/`WA_token_protected` must go through them (both `main.py` and `connect.py` do). `_get_wa_token()` transparently migrates a legacy plaintext `WA_token` to the protected `WA_token_protected` field on first read; a blob that fails to unprotect (corrupted, or copied to a different Windows user/machine) is treated exactly like "no token saved" — never a crash. Falls back to plaintext only if DPAPI genuinely isn't available. `secret.key` (the Fernet key backing DB payload encryption) is unrelated and still a plain file — that one only protects data at rest against a *different* machine/context reading the file, not against code running as the same user.
