...
Em regra não constam na documentação acima (que espelha o código do Gabriel); a exceção
é o `pl.json`, que virou parte integrante do programa e por isso também está documentado
lá. São preservados e evoluem aqui:
...
- `client/languages/pl.json` — idioma polonês. Exclusivo deste fork apenas quanto à
  origem: é um idioma **de primeira classe** do programa, selecionável nas configurações
  como os outros quatro e já registrado em `languages/language_map.json`. Toda string
  nova de interface entra nele também — vale a regra "adicione a chave nos cinco
  arquivos" da seção Paths, config, i18n acima, não a condição de extra opcional que o
  resto deste apêndice descreve. Chave ausente aqui não cai para o pt-BR: aparece na
  tela como o nome cru da chave (ver `I18n.t()`).
