...
# WinZapp's wppconnect-server overlay is discovered dynamically from the
# shipped api_patches/ directory. _PRESERVE and _KEEP_RUNTIME stay explicit
# because they describe installation policy rather than patch membership.
...
class ApiSetupDialog(wx.Dialog):
    ...
    def _patch_wppconnect_host_layer(wppconnect_api_dir: str) -> bool:
        ...
        try:
            ...
            for note in notes:
                log = logging.warning if "DID NOT MATCH" in note else logging.info
                log("[api_setup] host.layer.js: %s", note)
            if not ok:
                return False
            if patched != source:
                with open(host_layer_path, "w", encoding="utf-8") as f:
                    f.write(patched)
                logging.info("[api_setup] Patched host.layer.js pairing/auth flow.")
            return True
        except Exception:
            logging.exception("[api_setup] Could not patch host.layer.js.")
            return False
    ...
    def _run_setup(self):
        ...
        patches_dir = resource_path("api_patches")
        patch_files = server_patch_files(patches_dir)
        ...
        try:
            if not modules_only:
                ...
                try:
                    ...
                    # see patch_files for why this is needed. This is only
