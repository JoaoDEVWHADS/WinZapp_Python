...
async function getMediaByMessage(req, res) {
  ...
  try {
    ...
    // Try direct WPP.chat.downloadMedia in page evaluate if message not found or missing media URL
    let mediaUrl = message ? (message.clientUrl || message.deprecatedMms3Url || message.url || message.directPath || message.mediaUrl) : null;
    if (!message || !mediaUrl) {
