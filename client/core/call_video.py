...
def camera_names(ffmpeg_output: str) -> list[str]:
    ...
    for line in ffmpeg_output.splitlines():
        ...
        if not in_video or 'Alternative name' in line:
            continue
        # Recent FFmpeg builds append a media-kind suffix, e.g.
        #   [dshow @ ...]  "Integrated Camera" (video)
        # and the spacing after the dshow prefix is not stable. Accept both
        # that format and the older quote-only form while we are inside the
        # DirectShow video-device section.
        match = re.search(r'\]\s+"([^"]+)"(?:\s+\(video\))?\s*        if match:
