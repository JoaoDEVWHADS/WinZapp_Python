...
def effective_unread_count(chat) -> int:
    """A chat's unread count reported by server/DB.

    Returns the integer unread count reported for the chat (chat.get("unreadCount")).
    Does not zero out the count just because local message history hasn't been
    fetched into memory yet.
    """
