...
So: do not mark a chat exhausted while a confirmed phone request is still
inside its reply window. Background deep backfill has its own stall cooldown,
...
class TestOnceTheReplyWindowHasPassed:
    ...
    def test_user_scroll_overrides_an_exhausted_cache(self):
        """A durable background conclusion must not permanently block an
...
class TestInteractivePendingContract:
    def test_successful_phone_request_returns_none_not_empty_list(self):
        stub = _make()
        assert stub.fetch_older_messages(JID, ANCHOR) is None
        assert stub.asks == [JID]

    def test_poll_only_empty_page_stays_pending_and_does_not_ask(self):
        stub = _make()
        assert stub.fetch_older_messages(
            JID, ANCHOR, allow_phone_request=False
        ) is None
        assert stub.asks == []
        assert JID not in stub._exhausted_chats
